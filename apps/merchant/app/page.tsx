...
export default function Page() {
  ...
  const refresh = useCallback(async () => {
    const response = await fetch(`${API_URL}/merchant/redemptions`);
  ...
  return (
    <main style={{ maxWidth: 720, margin: "48px auto", padding: 24, fontFamily: "sans-serif" }}>
      ...
      <section style={{ border: "1px solid #ddd", borderRadius: 16, padding: 24 }}>
        <p>測試使用者：user-demo</p>
        <p>任務：完成一餐蔬食</p>
        <Button type="button" onClick={redeem}>確認核銷</Button>
      </section>
      <p aria-live="polite">{message}</p>
      <h2>核銷紀錄</h2>
      {records.length === 0 ? <p>尚無紀錄</p> : records.map((record) => (
        <article key={record.id} style={{ borderTop: "1px solid #ddd", padding: "12px 0" }}>
          <strong>{record.id}</strong>
          <p>{record.userId}｜+{record.starsGranted} ⭐｜+{record.energyGranted} 能量</p>
        </article>
      ))}
    </main>
  );
