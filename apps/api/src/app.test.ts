...
async function onboardMerchant(app: Awaited<ReturnType<typeof setup>>) {
  ...
  const approved = await app.inject({
    method: "POST",
    url: `/merchant-applications/${application.id}/review`,
    headers: adminHeaders,
    payload: { decision: "approve", reviewerId: "admin-demo" },
  });
...
test("完整平台 MVP：店家加入、平台審核、玩家接任務、店家核銷", async () => {
  ...
  const accepted = await app.inject({
    method: "POST",
    url: `/missions/${mission.id}/accept`,
    payload: { userId: "user-demo" },
  });
  ...
  const redeemed = await app.inject({
    ...
    payload: {
      ...
      merchantId,
      idempotencyKey: "onboarding-flow-0001",
    },
  });
...
test("相同核銷請求重送不會重複發獎勵", async () => {
  ...
  const request = {
    ...
    url: "/redemptions",
    headers: merchantHeaders,
    payload: { userId: "user-demo", missionId: mission.id, merchantId: application.merchantId, idempotencyKey: "same-request-0001" },
  };
