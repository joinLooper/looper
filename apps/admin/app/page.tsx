...
import type { AdminEconomyResponse, AdminOverview, EconomySettings, MerchantApplication, MerchantApplicationReviewDecision, MerchantPlan, MerchantProfile } from "@looper/types";
...
import { adminEconomyRequest, canManageEconomySettings, canManageMerchantPlans, canReadAdminEconomy, classifyAdminEconomyError, economySettingsUpdateRequest, merchantPlanUpdateRequest } from "./admin-economy-flow";
...
function actionLabel(action: string) {
  ...
  if (action === "merchant.plan_updated") return "店家方案更新";
  return action;
}
...
export default function Page() {
  ...
  const canReadMerchantPlans = hasPlatformPermission(adminSession?.context ?? null, "platform.merchant_plan.read");
  const canReadEconomySettings = hasPlatformPermission(adminSession?.context ?? null, "platform.economy.read");
  const canLoadEconomy = canReadAdminEconomy(adminSession?.context ?? null);
  const contextCanManageMerchantPlans = canManageMerchantPlans(adminSession?.context ?? null);
  const contextCanManageEconomySettings = canManageEconomySettings(adminSession?.context ?? null);
  ...
  const [merchantPlanPermissionBlocked, setMerchantPlanPermissionBlocked] = useState(false);
  const [economyPermissionBlocked, setEconomyPermissionBlocked] = useState(false);
  const overviewRequestVersion = useRef(0);
  const reviewRequestVersion = useRef(0);
  const merchantPlanRequestVersion = useRef(0);
  const economyRequestVersion = useRef(0);
  const canReviewApplications = contextCanReviewApplications && !reviewPermissionBlocked;
  const canManagePlans = contextCanManageMerchantPlans && !merchantPlanPermissionBlocked;
  const canManageEconomy = contextCanManageEconomySettings && !economyPermissionBlocked;
  ...
  const refresh = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      const safeOverview: AdminOverview = {
        ...data,
        merchantApplications: canReadApplications ? data.merchantApplications : [],
        merchants: canReadMerchantPlans ? data.merchants : [],
        merchantPlans: canReadMerchantPlans ? data.merchantPlans : [],
        economySettings: canReadEconomySettings ? data.economySettings : null,
        levelDefinitions: canReadEconomySettings ? data.levelDefinitions : [],
      };
      setOverview(safeOverview);
      if (canLoadEconomy) {
        const economyResponse = await fetch(`${API_URL}/admin/economy`, adminEconomyRequest);
        if (version !== overviewRequestVersion.current) return false;
        if (!economyResponse.ok) {
          const error = classifyAdminEconomyError(economyResponse.status);
          setEconomy(null);
          setSettingsForm(null);
          ...
          setMessage(error === "forbidden" ? "目前帳號沒有方案或經濟設定讀取權限。" : "暫時無法讀取方案與經濟設定。");
          return false;
        }
        setEconomy(await economyResponse.json() as AdminEconomyResponse);
      }
  ...
  }, [adminSession, canLoadEconomy, canLoadOverview, canReadApplications, canReadEconomySettings, canReadMerchantPlans]);
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      merchantPlanRequestVersion.current += 1;
      economyRequestVersion.current += 1;
    };
  }, [refresh]);
  useEffect(() => {
    reviewRequestVersion.current += 1;
    ...
    setMerchantPlanPermissionBlocked(false);
    setEconomyPermissionBlocked(false);
    if (!canReadApplications) {
      setOverview((current) => current ? { ...current, merchantApplications: [] } : current);
    }
    if (!canReadMerchantPlans) {
      setOverview((current) => current ? { ...current, merchants: [], merchantPlans: [] } : current);
      setEconomy(null);
      setSettingsForm(null);
    }
    if (!canReadEconomySettings) {
      setOverview((current) => current ? { ...current, economySettings: null, levelDefinitions: [] } : current);
      setEconomy(null);
      setSettingsForm(null);
    }
  }, [adminSession?.context.accountId, canReadApplications, canReadEconomySettings, canReadMerchantPlans, contextCanManageEconomySettings, contextCanManageMerchantPlans, contextCanReviewApplications]);
  useEffect(() => {
    if (!economy?.settings) return;
    setSettingsForm(Object.fromEntries(settingFields.map((field) => [field.key, String(economy.settings[field.key])])) as Record<SettingKey, string>);
  }, [economy?.settings.version]);
  ...
  async function updatePlan(merchantId: string, merchantPlan: MerchantPlan) {
    if (isBusy || !adminSession || !canManagePlans) return;
    const version = ++merchantPlanRequestVersion.current;
    setIsBusy(true);
    setMessage("正在更新店家方案...");
    try {
      const response = await fetch(`${API_URL}/merchants/${merchantId}/plan`, merchantPlanUpdateRequest(merchantPlan));
      if (version !== merchantPlanRequestVersion.current) return;
      if (response.ok) {
        const refreshed = await refresh();
        if (version === merchantPlanRequestVersion.current) setMessage(refreshed ? "店家方案已更新，新核銷會採用新星星額度。" : "操作已完成，但資料更新失敗");
        return;
      }
      const error = classifyAdminEconomyError(response.status);
      if (error === "unauthenticated") {
        setOverview(null);
        setEconomy(null);
        setSettingsForm(null);
        ...
        setEconomy(null);
        setSettingsForm(null);
        setMerchantPlanPermissionBlocked(true);
      } else if (error === "not_found") {
        await refresh();
      }
      if (version === merchantPlanRequestVersion.current) setMessage(error === "not_found" ? "店家資料已更新，請重新選擇。" : "店家方案更新失敗，請稍後再試。");
    } catch {
      if (version === merchantPlanRequestVersion.current) setMessage("店家方案更新失敗，請稍後再試。");
    } finally {
      if (version === merchantPlanRequestVersion.current) setIsBusy(false);
    }
  }

  async function saveSettings(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!economy || !settingsForm || isBusy || !adminSession || !canManageEconomy) return;
    const version = ++economyRequestVersion.current;
    setIsBusy(true);
    setMessage("正在儲存經濟設定...");
    try {
      ...
      const response = await fetch(`${API_URL}/admin/economy-settings`, economySettingsUpdateRequest(payload, economy.settings.version));
      if (version !== economyRequestVersion.current) return;
      if (response.ok) {
        const data = await response.json() as { changed: boolean; settings: { version: number } };
        const refreshed = await refresh();
        if (version === economyRequestVersion.current) {
          setMessage(refreshed ? (data.changed === false ? "設定沒有變更。" : `經濟設定已更新到 v${data.settings.version}。`) : "操作已完成，但資料更新失敗");
        }
        return;
      }
      const error = classifyAdminEconomyError(response.status);
      if (error === "unauthenticated") {
        ...
        setEconomy(null);
        setSettingsForm(null);
        setEconomyPermissionBlocked(true);
      } else if (error === "conflict") {
        await refresh();
      }
      if (version === economyRequestVersion.current) setMessage(error === "conflict" ? "設定版本已更新，已重新載入正式版本。" : "經濟設定儲存失敗，請稍後再試。");
    } catch {
      if (version === economyRequestVersion.current) setMessage("經濟設定儲存失敗，請稍後再試。");
    } finally {
      if (version === economyRequestVersion.current) setIsBusy(false);
    }
  }
  ...
  return <main className="admin-shell">
    ...
    {canLoadEconomy && economy ? <section className="panel settings-panel">
      <div className="panel-header"><h2>核心經濟設定</h2><span className="panel-count">v{economy.settings.version}・{economy.settings.updatedBy}</span></div>
      <form className="settings-form" onSubmit={saveSettings}>
        <p className="settings-meta">最後更新：{new Date(economy.settings.updatedAt).toLocaleString("zh-TW")}</p>
        <div className="settings-field-grid">
          {settingFields.map((field) => <label className="settings-field" key={field.key}><span>{field.label}<small>{field.unit}</small></span><input type="number" step={field.step ?? "1"} value={settingsForm?.[field.key] ?? ""} onChange={(event) => setSettingsForm((current) => current ? { ...current, [field.key]: event.target.value } : current)} disabled={isBusy || !canManageEconomy} /></label>)}
        </div>
        {canManageEconomy ? <div className="settings-actions"><Button className="action-primary" type="submit" disabled={isBusy || !settingsForm}>{isBusy ? "儲存中..." : "儲存經濟設定"}</Button></div> : null}
      </form>
      <div className="settings-grid">
        {economy.merchantPlans.map((plan) => <span key={plan.plan}>{plan.label}: ⭐{plan.rewardStarAmount}</span>)}
      </div>
      <div className="settings-grid">
        {economy.levelDefinitions.map((level) => <span key={level.level}>LV.{level.level}: total EXP {level.requiredTotalExp}, ⭐{level.rewardStars}, max_energy +{level.maxEnergyIncrease}</span>)}
      </div>
    </section> : <section className="panel settings-panel"><div className="empty-state"><strong>方案與經濟設定未載入</strong><span>需要正式方案與經濟設定讀取權限。</span></div></section>}
    ...
    <div className="workspace-grid">
      ...
      <div style={{ display: "grid", gap: 18 }}>
        {canReadMerchantPlans ? <section className="panel"><div className="panel-header"><h2>已啟用合作店家</h2><span className="panel-count">{overview?.merchants.length ?? 0} 家</span></div><div className="panel-body">{overview?.merchants.length ? <div className="merchant-list">{overview.merchants.map((merchant) => <article className="merchant-row" key={merchant.id}><div><strong>{merchant.storeName}</strong><br /><small>{categoryLabel(merchant)}・{merchant.address}<br />方案：{merchant.merchantPlan}・每次 ⭐{merchant.rewardStarAmount}・CO₂ 固定 800g</small></div>{canManagePlans ? <select value={merchant.merchantPlan} onChange={(event) => updatePlan(merchant.id, event.target.value as MerchantPlan)} disabled={isBusy}>{economy?.merchantPlans.map((plan) => <option value={plan.plan} key={plan.plan}>{plan.label}</option>)}</select> : <span>{merchant.merchantPlan}</span>}</article>)}</div> : <div className="empty-state"><strong>尚無合作店家</strong><span>通過審核後會建立任務與店家後台。</span></div>}</div></section> : <section className="panel"><div className="empty-state"><strong>目前帳號沒有店家方案讀取權限</strong><span>店家方案明細不會傳送或顯示。</span></div></section>}
